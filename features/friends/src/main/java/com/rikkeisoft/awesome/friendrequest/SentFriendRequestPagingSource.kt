    ...
    override suspend fun load(params: LoadParams<DocumentSnapshot>): LoadResult<DocumentSnapshot, FriendRequestUI> =
        withContext(Dispatchers.IO) {
            try {
                ...
                var query = db.collection("friends_request")
                ...
                val friendRequests = snapshot.documents.mapNotNull { it.toObject(FriendRequest::class.java) }

                val friendRequestUIs = friendRequests.map { friendRequest ->
                    async {
                        // For sent requests, we want to display the receiver's info
                        val receiverUser = db.collection("users").document(friendRequest.receiverId).get().await()
                            .toObject(User::class.java)
                        FriendRequestUI(
                            id = friendRequest.id,
                            sender = receiverUser, // Using sender field to hold receiver info for UI display
